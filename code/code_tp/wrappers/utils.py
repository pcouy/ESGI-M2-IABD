...
class LogScaleObs(gym.ObservationWrapper):
    ...
    def __init__(self, env, *args, **kwargs):
        ...
        self.observation_space = gym.spaces.Box(
            low=self.t(self.o.low), high=self.t(self.o.high), shape=self.o.shape
        )
    ...
    def t(self, x):
        ...
        l = np.min(
            np.stack(
                (np.abs(self.o.low), np.abs(self.o.high), np.zeros_like(self.o.low)),
                axis=1,
            ),
            axis=1,
        )
...
class TabularObservation(gym.ObservationWrapper):
    ...
    def observation(self, s):
        ...
        for i, (low, high, n, obs) in enumerate(
            zip(self.old_obs_space.low, self.old_obs_space.high, self.n_levels, s)
        ):
            assert high >= low
            try:
