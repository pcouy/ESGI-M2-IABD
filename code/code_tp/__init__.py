...
def dict_to_dirname(x):
    return "-".join(
        ["{}_{}".format(k, v.__name__ if type(v) is type else v) for k, v in x.items()]
    )
...
def create_agent_from_env(
    ...
    value_args.update({"env": env})
    value_function = value_class(**value_args)

    policy_args.update({"value_function": value_function})
    policy = policy_class(**policy_args)

    agent_args.update({"env": env, "value_function": value_function, "policy": policy})
