...
class SnakeFeatureObservation(gym.ObservationWrapper):
    ...
    def __init__(self, env, grid_size, food_direction="unit_vector", *args, **kwargs):
        ...
        * `food_direction`: prend les valeurs `"unit_vector"` ou `"angle"`, détermine si la direction
        ...
        self.observation_space = gym.spaces.flatten_space(gym.spaces.Tuple((
            gym.spaces.Box(low=0, high=grid_size, shape=(9,)),
            gym.spaces.Box(low=-1, high=1, shape=(2,)) if food_direction == "unit_vector" else
        ...
        for dx in (-1,0,1):
            for dy in (-1,0,1):
                if dx==0 and dy==0:
                    ...
                    d = 1
                    while (
                        scan_pos[0] >= 0 and scan_pos[0] < w and scan_pos[1] >= 0 and scan_pos[1] < h
                    ) and (
                        np.all(state[tuple(scan_pos)] == self.OC.empty) or
                        np.all(state[tuple(scan_pos)] == self.OC.apple)
                    ):
