...
class TargetValueAgent(QLearningAgent):
    ...
    def train_with_transition(
        ...
        if self.discrete_updates and self.training_steps % self.target_update == 0:
            self.target_value_function.import_f(self.value_function.export_f())
        elif not self.discrete_updates:
            self.target_value_function.mix_with(
                self.value_function, tau=self.target_update
            )
...
class DoubleQLearning(TargetValueAgent):
    ...
    def eval_state_batch(self, states, prev_actions=None):
        ...
        values = self.target_value_function.call_batch(
            states, selected_actions, prev_actions
        )
